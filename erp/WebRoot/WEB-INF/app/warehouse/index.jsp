    ...
    <form id="product" class="layui-form detailbox" action="../product/add" >
      <div class="layui-form-item">
        ...
        <div class="layui-input-block">
          <input type="text" name="wname" id="wname" required  lay-verify="required" placeholder="请输入标题" autocomplete="off" class="layui-input">
        </div>
      </div>
      <div class="layui-form-item">
      ...
      <div class="layui-form-item">
        ...
        <div class="layui-input-block">
          <select name="manageUser" id="manageUser"  lay-verify="required">
            <option value=""></option>
          </select>
        </div>
      </div>
  ...
  $(function(){
    layui.use(['layer', 'table','form'], function(){
        ...
        table.render({
            ...
            ,page: true //开启分页
            ,cols: [[
                {field: 'wid', title: '仓库编号', width:'100', sort: true}
                ,{field: 'wname', title: '仓库名称', sort: true}
                ,{field: 'type',title: '仓库类型', sort: true}
                ,{field: 'manageUser', title: '负责人', sort: true,width:80}
                ,{field: 'address', title: '地址', sort: true}
//                ,{field: 'createUser', title: '创建者', sort: true}
                ,{field: 'createDate' ,title: '创建时间', sort: true,width:175}
                ,{field: 'remark' ,title: '备注', sort: true,width:175}
                ...
                table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if(layEvent === 'detail'){ //查看
                        ...
                        layer.confirm('真的删除行么', function(index){
                            ...
                            layer.close(index);
                            deleteProductByid(data.wid,function (data) {
                        ...
                        layer.open({
                            ...
                            btn:['保存', '取消'],
                            success:function () {
                                buildMangeUser(function(){
                                    $("#wname").val(data.wname);
                                    $("#type").find("option[text='"+data.type+"']").attr("selected",true);
                                    $("#manageUser").find("option[text='"+data.manageUser+"']").attr("selected",true);
    ...
    function buildMangeUser(cb){
        $.ajax({
            ...
            success: function (obj) {
               if(obj.flag){
                   var userStr = Mustache.render('{{#object}} <option value="{{uid}}">{{realname}}</option>{{/object}}',obj)
                   $('#manageUser').html(userStr);
                   cb();
               }
            }
        });
